...
bpm_results = pd.DataFrame(columns=['Subject', 'POS BPM', 'GRGB BPM'])
...
for subject_name in subject_names:
    ...
    print(f"Subject {subject_name}: Total Frames = {len(png_list)}, RGB = {len(mean_rgb)}")

    # Store the BPM result in the DataFrame
    new_row = pd.DataFrame({'Subject': [subject_name], 'POS BPM': [rPPG_bpm], 'GRGB BPM': [GRGB_bpm]})
